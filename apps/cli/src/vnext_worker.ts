...
export async function executeVNextWorker(
  ...
  if (!workerId) throw new Error(`vnext worker ${input.action} requires --worker-id`);
  ...
  try {
    ...
    if (executionWorker) {
      if (["completed", "failed", "needs_input"].includes(executionWorker.status)) {
        return envelope(executionWorker.status, executionWorker.id, executionWorker, "execute");
      }
...
function workerInspectionEvidence(
  ...
  return {
    ...
    lease_ordinal: worker.lease_ordinal,
