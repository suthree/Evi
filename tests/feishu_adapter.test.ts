...
test("bound Feishu group bot mention executes without leaking the mention into the task", async () => {
  ...
  try {
    ...
    await adapter.handleInboundEvent(feishuEvent({
      messageId: "om_group_mention_bind",
      chatType: "group",
...
test("operator capabilities command replies with local capability catalog without running the agent", async () => {
  ...
  try {
    ...
    assert.match(fullText, /dispatch_failure_kind values are dispatch_limit_exceeded, input_contract_failed, terminal_completion_claim, terminal_response_action, or none/);
    assert.match(fullText, /none means no dispatch-layer failure, not delegated success/);
    assert.match(fullText, /result_failure_kind values are dispatch_limit_exceeded, input_contract_failed, terminal_completion_claim, terminal_response_action, delegated_output_contract_failed, delegated_model_request_failed, or none/);
...
test("operator live run trace commands read bounded run metadata without running the agent", async () => {
  ...
  try {
    ...
    await fixture.store.appendJsonl("memory/episodes/events.jsonl", {
      ...
      summary: "Delegated result: action_id=action_delegate_trace_feishu; round=1; sequence=1; task_chars=55; context_chars=99; model_invoked=true; contract_status=failed; dispatch_failure_kind=none; result_failure_kind=delegated_output_contract_failed; ok=false.",
    ...
    assert.match(transport.sent[1].text, /model_invoked: true/);
    ...
    assert.match(replayDetailText, /model_invoked: true/);
