    ...
    general_delegation_loop: {
      ...
      runner_enforcement_contract: {
        ...
        input_contract: [
          ...
          "validateDelegationTaskBoundary requires explicit bounded analysis intent and rejects direct fix/update/edit/patch/commit, command/test execution, tool, write, mutation, completion, expert, or multi-agent scheduling requests"
        ],
        ...
        completion_gate: [
          ...
          "delegatedIndependentEvidenceCheck requires later harness-known non-delegated verification refs after the latest delegated result, successful write/run evidence only counts when its harness-known ref is cited, and failed delegation still requires later successful write/run recovery evidence plus bound non-delegated verification refs"
        ]
      },
      recovery_contract: {
      ...
      replay_audit_contract: {
        ...
        required_metadata: ["action_id", "dispatch_failure_kind", "result_failure_kind"],
        checks: [
...
test("context bundle includes bounded GA project design plan", async () => {
  ...
  try {
    ...
    assert.match(rendered.markdown, new RegExp(`delegation_loop: action=delegate_agent; stage=active; max_per_round=${DELEGATE_AGENT_MAX_ACTIONS_PER_ROUND}; dispatch_kind=dispatch_failure_kind; result_kind=result_failure_kind; task_max=1000; context_max=12000; result=240/2000; runner=parseDelegationRequest validates strict task/context payloads before delegated model dispatch\\+validateDelegationTaskBoundary requires explicit bounded analysis intent and rejects direct fix/update/edit/patch/commit, command/test execution, tool, write, mutation, completion, expert, or multi-agent scheduling requests; validateDelegationContextBoundary rejects context grants for completion, expert scheduling, multi-agent orchestration, model fan-out, hidden memory, raw delegated artifacts, unstated repo state, context expansion, or invented evidence refs; gate=delegatedResultsCheck fails a done claim when delegated failure lacks later main-harness recovery evidence\\+delegatedVerificationRefs rejects delegated self-report refs as completion proof\\+delegatedIndependentEvidenceCheck fails a done claim after delegation without later harness-known non-delegated verification refs after the latest delegated result; successful write/run evidence only counts when its harness-known ref is cited, and failed delegation still requires later successful write/run recovery evidence plus bound non-delegated verification refs; recovery=failed delegated results may only guide a later main-harness model round as sanitized observation; replay=delegated_action_coverage\\+delegated_dispatch_metadata\\+delegated_dispatch_failure_kind\\+delegated_dispatch_round_limit\\+delegated_result_failure_kind\\+delegated_results; authority=main harness verifies delegated results before they influence a done claim; defer=no expert personas,no autonomous multi-agent scheduling`));
    ...
    assert.match(rendered.markdown, /selection: ready; source_kind=verified_artifact \| source_status=verified \| source_artifact_quality=attention/);
...
test("context bundle includes bounded live run trace without raw artifacts", async () => {
  ...
  try {
    ...
    await fixture.store.appendJsonl("memory/episodes/events.jsonl", {
      ...
      summary: "Delegated result: action_id=action_delegate_trace_context; round=1; sequence=1; task_chars=44; context_chars=88; contract_status=failed; dispatch_failure_kind=none; result_failure_kind=delegated_output_contract_failed; ok=false.",
    ...
    assert.match(rendered.markdown, /delegated_dispatch: round=1 sequence=1 status=failed ok=false dispatch_failure_kind=none result_failure_kind=delegated_output_contract_failed task_chars=44 context_chars=88 action_id=action_delegate_trace_context ref=memory\/episodes\/session_live_trace_context-delegated_result_invalid\.json/);
...
test("live runner rejects contradictory delegate context authority grants", async () => {
  ...
  try {
    ...
    assert.match(delegated.error ?? "", /context must not grant tool\/write\/mutation, completion, expert, or multi-agent scheduling authority/);
    assert.equal(delegated.raw_output_preview, "");
    assert.equal(report.verification_status, "failed");
    assert.equal(report.verified, false);
    assert.equal(report.checks.find((check) => check.id === "delegated_results")?.status, "fail");
  ...
  private nextMainEnvelope(request: ModelRequest): Record<string, unknown> {
    ...
    if (this.mainCalls === 2) {
      ...
      this.sawSanitizedDelegationObservation = delegatedSection.includes('"contract_status": "passed"')
        ...
        && !delegatedSection.includes("Use latest context notes to analyze bounded risks.")
        && !delegatedSection.includes(BOUNDED_DELEGATE_CONTEXT);
      return stateWriteEnvelope();
    }
    if (this.mainCalls > 2) {
...
class ContradictoryDelegationBoundaryThenDoneModel implements ModelClient {
  ...
  private nextMainEnvelope(request: ModelRequest): Record<string, unknown> {
    ...
    if (this.mainCalls > 1) {
      ...
      this.sawFailedBoundaryObservation = delegatedSection.includes('"contract_status": "failed"')
        && delegatedSection.includes('"result_failure_kind": "input_contract_failed"')
        && delegatedSection.includes("delegate_agent.payload.context must not grant tool/write/mutation, completion, expert, or multi-agent scheduling authority")
...
const BOUNDED_DELEGATE_CONTEXT = "No tool, write, or mutation authority is available; completion remains with the main harness. Return JSON with summary and findings_text.";
const DELEGATE_CONTEXT_WITH_GENERIC_OUTPUT_SHAPE = "No tool, write, or mutation authority is available; completion remains with the main harness. Return JSON with summary and findings.";
  ...
  return {
    ...
    actions: [{
      ...
      payload: {
        ...
        context: "Read-only analysis only; no use_tool or file.write_repo side effects. Final success is verified by the main harness. Return JSON with summary and findings_text."
      }
    }],
    completion_claim: {
      status: "not_done",
...
function latestContextAnalysisDelegateEnvelope(): Record<string, unknown> {
  return {
    ...
    actions: [{
      ...
      payload: {
        task: "Use latest context notes to analyze bounded risks.",
        context: BOUNDED_DELEGATE_CONTEXT
      }
    }],
    completion_claim: {
