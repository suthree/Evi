...
test("capability catalog mirrors core tool and harness action contracts", () => {
  ...
  assert.equal(catalog.catalog_version, "2026-07-18");
  ...
  assert.equal(catalog.refs.includes("packages/runtime/src/goal_tool_competence.ts"), true);
  ...
  const toolCompetence = memoryAndLearning?.capabilities.find((capability) => capability.id === "goal.tool_competence");
  assert.equal(toolCompetence?.layer, "core_runtime");
  assert.equal(toolCompetence?.category_layer, "local_learning");
  assert.equal(toolCompetence?.effective_layer, "core_runtime");
  assert.equal(toolCompetence?.refs?.includes("packages/runtime/src/goal_tool_competence.ts"), true);
  assert.equal(toolCompetence?.boundaries?.some((boundary) => boundary.includes("persists no new ledger")), true);
  assert.equal(toolCompetence?.boundaries?.some((boundary) => boundary.includes("association only") && boundary.includes("never causal attribution")), true);
  assert.equal(toolCompetence?.boundaries?.some((boundary) => boundary.includes("current evidence") && boundary.includes("authoritative")), true);
  assert.equal(readModels?.capabilities.some((capability) => capability.id === "project.design_contract"), false);
  assert.equal(memoryAndLearning?.capabilities.some((capability) => capability.id === "self_evolution.scorecard"), false);
  assert.equal(memoryAndLearning?.capabilities.some((capability) => capability.id === "self_evolution.iterations"), false);
...
test("capability acceptance audit records next-version gates without execution authority", () => {
  ...
  assert.equal(audit.gates.some((gate) => gate.id === "agent_harness" && gate.evidence_refs.includes("packages/core/src/live_run_trace.ts")), true);
  ...
  assert.equal(audit.gates.some((gate) => gate.id === "agent_harness" && gate.boundaries.some((boundary) => boundary.includes("delegated dispatch failure metadata"))), true);
