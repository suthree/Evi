...
Runtime state under `.runtime/` records what happened in a repo-local run. The
local active vault under `LOCAL_RUNTIME_HOME` is the runtime write target for
promoted procedures. Repository `vault/` keeps seed examples, test fixtures,
and provenance-backed development material.
...
├── examples/            # curated examples distilled from local runs
├── registry/
...
- Keep raw `.runtime/` reports out of git; promote only curated examples or fixtures into `vault/`.
