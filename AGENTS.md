...
- Use an isolated branch/worktree when a source change needs it. The current `GoalExecutionWorkspace` still accepts legacy `codex/issue-N-slug` names; this is compatibility only and does not require a live GitHub Issue. Keep any replacement of that naming rule in a separately verified runtime slice.
