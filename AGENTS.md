...
- Keep model-facing instruction and identity files in English when that makes the runtime contract clearer. Examples: `AGENTS.md`, `core/soul.md`, and related prompt/context files. Frozen generated archive guidance is historical evidence, not a model-facing entrypoint.
...
- English repository documents are the only default route for models and operators. Localized documents may be retained as explicitly requested material, but are not default context and do not require a companion update.
...
- Start with `README.md`, `docs/CURRENT_DIRECTION.md`, `docs/INDEX.md`, `memory/index.md`, `core/soul.md`, and `core/memory.md`. `docs/CURRENT_DIRECTION.md` is the default architecture and planning route; source, tests, and verified runtime evidence decide implementation facts.
- A concrete implementation task may explicitly select a long document by subject or identifier. Retrieve a dated record only by explicit identifier when its evidence is needed.
...
- A task explicitly about active exploration, content publishing, image generation, Xiaohongshu adapters, or feedback capture may select `docs/ACTIVE_EXPLORATION.md`.
- Before source-mutating self-evolution work, establish an Environment Baseline: classify inherited work, preserve its evidence, assign current changes to an owner and disposition, verify worktree/branch/stash state, and check runtime identity and health. Do not replace this with a state wipe or a superficial clean status. Historical records are evidence only; never refresh or use generated archive context as active instruction.
...
| `docs/adr/` | Named architectural and governance evidence when a task selects it | Default direction, runtime state, active Goal state, or hidden reasoning |
| Frozen archive records | Historical specs, tasks, decisions, and evidence | Active governance, default context, or generated agent instruction |

## Work Discipline

- Inspect current code and runtime state before making claims about this repository.
- Start self-evolution work from the active task, Harness, canonical evidence, and independently verified outcome evidence. Use the applicable Decision Owner to scale controls by scope, evidence, risk, reversibility, recovery, and current operator intent. Durable code, dependency, deployment, or external effects require explicit scope, evidence, verification, and rollback or retirement; they do not require a default Issue or archive task.
...
- Every source-mutating work item uses one isolated branch/worktree for its full delivery lineage; later sessions and tools reuse that bound worktree rather than creating one per session. The root checkout stays clean on protected `develop`; it accepts source only through a verified pull-request merge, never a direct commit or push. The shared Evi control state is `~/.local-runtime/state/evi`, not a project-local `.runtime/` directory. Keep any runtime naming-rule replacement in a separately verified slice.
...
- A delegated `codex.run` must reserve its exact task/effect/action-digest dispatch record before the child starts. Only a matching terminal child record may reconcile an unknown effect outcome; a missing, active, invalid, or mismatched record leaves the effect paused and never authorizes replay. Raw prompts must not enter the durable dispatch record.
...
- Keep active project direction in `docs/CURRENT_DIRECTION.md` and the stable docs it routes to. Preserve archive material unchanged as historical evidence; do not refresh generated archive context or route new work through it.
