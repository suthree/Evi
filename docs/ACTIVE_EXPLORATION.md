...
`content daily` writes `content/daily/YYYY-MM-DD.json` for the legacy default
track, or `content/daily/<track>/YYYY-MM-DD.json` when `--track` is supplied.
...
or ticker set, the resident loop falls back to the legacy single daily job. With
