...
- 新 dispatch 还会持久化由同一 `parseDelegationRequest` 推导的输入有效性、规范化 `task/context` 长度和 SHA-256 digest。delegated result artifact 中的 `task` 固定为审计标记，不保存 raw task text；trace 从 declaring envelope 重算同一份有界期望。任一 action、有效性、长度或 digest 不一致，以及历史事件缺少新 metadata，都会保持 warning/unknown，不能把 delegated completion gate 升级为 clean。digest 只用于对账，不展示 raw delegated task/context。
