...
model dispatch 与 persisted Pi tool-call protocol 恢复。显式 CLI-only 的 read-only canary
继续作为诊断入口；独立的稳定 Goal-free CLI ingress 已增加 durable Session-to-Run binding
与每 Run 不可变 Execution Lock。两者都未连接生产 Web/IM ingress，也没有切换当前 v0.2 部署。
...
gate；显式、隔离 SQLite 的 read-only ingress canary 仍是诊断入口，稳定 Goal-free CLI 已实现
durable Session binding 和 Execution Lock，但尚未接入 Web/IM 或部署。稳定方向由项目文档和
ADR 记录；当前实现以源码、测试和 live health 为准。GitHub Issue、PR、branch
