...
- completion report 会写入 `verification_evidence_refs`，只记录成功 tool result id 和 tool artifact ref 的来源、轮次、tool、side effect、claimed、post-delegation independent/recovery 标记；state-only harness actions 和 delegated result refs 不进入完成证明 lineage。trace/replay 可用这些 bounded metadata 复核完成证明，不读取 raw tool body、delegated artifact body 或 final response body。
