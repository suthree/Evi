...
- `codex.run`：位于通用 `command.run` 之上的独立 typed coding execution tool。它只允许 `gpt-5.6-sol`、`fast` profile、`xhigh` reasoning、`fast` service tier、`read-only` 或 `workspace-write` sandbox，以及 `never` approval；会实时校验同一 Git common dir 下已注册的 sibling isolated worktree、base、branch 和 cwd，以不可变 digest 绑定 new/resume authority，解析有界 JSONL 与严格结构化状态，对整个 POSIX process group 做 TERM/KILL 清理，并从运行前后 live Git status 记录 tracked 与 untracked changed paths。
