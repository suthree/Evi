...
- completion report 会写入 `verification_evidence_refs`，只记录成功 tool result id 和 tool artifact ref 的来源、轮次、tool、side effect、claimed、post-delegation independent/recovery 标记；失败委派后，只有被 done claim 引用的成功 write/run tool result id 或 tool artifact ref 才能标记为 recovery，未引用的写入和只读 ref 都不能恢复失败委派；state-only harness actions 和 delegated result refs 不进入完成证明 lineage。trace/replay 会用 bounded metadata 重新校验 independent 标记必须满足 claimed、存在于 done claim、成功且晚于最新 delegation，并额外校验 recovery 标记必须是 write/run、晚于最新 failed delegation 且确有 failed delegated result，不读取 raw tool body、delegated artifact body 或 final response body。
