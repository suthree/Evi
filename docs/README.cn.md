...
- `delegate_agent` 的 task 可以仅基于显式 payload context 或 named evidence refs，分析命令、测试或脚本是否已经执行；这仍是只读分析。直接要求 delegated subagent 运行、执行，或用 `test the project`、`检查构建`、`read files`、`搜索代码库` 这类裸动词执行测试/构建/读取/搜索的任务，都会在 dispatch 前被拒绝。边界解析会先移除不可见 Unicode format control，不能用 `f\u200bix` 这类拆分来隐藏修复、Git 或权限意图。
