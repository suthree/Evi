...
- `delegate_agent`：每个 model round 最多分发一个有界分析子任务，payload 只能包含 `task/context`；`task` 必须显式请求有界 analysis、critique、review、inspection、comparison、summarization 或 evaluation，不能是笼统 handoff，不能把分析和直接 fix/repair/update/edit/patch/commit 意图混在一起，也不能要求 delegated subagent 运行命令、测试、构建或包管理脚本，执行工具、写入或突变状态、决定完成、调度专家或多 agent 工作；`context` 必须显式声明 delegated subagent 没有工具/写入/突变权限，完成判断仍归主 harness，写清期望输出形状是 `summary/findings_text`，并声明 delegated analysis 只能使用显式 payload context 或命名 evidence refs，也不能一边否认权限一边授予工具、写入、突变、完成权、专家调度、多 agent 编排、模型 fan-out、hidden memory、raw delegated artifact、未声明 repo 状态、context expansion 或自造 evidence ref。结果必须是受 schema 约束的 `summary/findings_text` 且遵守输出上限。它不是任务执行调度器，不能授予工具、状态写入、专家调度或完成判断权。超出的 delegate action，或违反 task/context 权限边界的输入，会记录为失败且不会调用子模型，并带有安全的 `dispatch_failure_kind`；没有分发层失败时也要显式记录 `dispatch_failure_kind=none`，这里的 `none` 只表示没有分发层失败，不表示 delegated result 成功。失败 delegated result 还会记录安全的 `result_failure_kind`，区分 dispatch limit、input contract、delegated output contract、delegated model request failure；成功 delegated result 显式记录 `result_failure_kind=none`，持久化结果和回灌观察都使用显式 `none` 而不是 `null`；replay audit 还会检查两层 failure kind 的语义配对，分发层失败必须镜像，delegated output/model 失败的 dispatch 层必须是 `none`，成功结果两层都必须显式 `none`。这些失败只能作为主 harness 后续恢复输入，不能触发自动 retry、模型 fan-out、专家调度或 delegated completion。成功 delegated result 的 `summary/findings_text/raw_output_preview` 和失败错误文本都会在持久化和回灌前脱敏；如果输出回显 raw delegated task/context，会作为 delegated output contract failure，且 raw preview 被抑制。回灌给主模型的是脱敏观察，不包含 raw delegated task/context/output preview 或 artifact body。即使 delegated result 通过 schema，它也只是 advisory self-report，不能作为 `completion_claim.verification_refs` 的完成证明；如果 `done` claim 之前使用过 delegated result，还必须在 `completion_claim.verification_refs` 中绑定 harness-known 的非 delegated tool result id / tool artifact ref，成功的 write/run 只有被 done claim 引用时才算完成证明，模型自造 ref 和 final response 本身都不算独立完成证明；如果 delegated result 失败，后续 `done` 必须同时具备主 harness 后续写入/运行恢复证据，以及绑定到 harness-known 非 delegated evidence 的 verification ref，否则仍失败；恢复后该失败只保留为 warning；`record_evidence`、`update_working_state`、`not_done` `propose_sop`、`propose_memory`、`request_audit`、`pause_autonomy` 等 state-only harness/governance action 只能留下状态观察、草稿或请求，不能作为 failed delegation recovery evidence；已绑定的 read-only tool ref 可以作为 done claim 的独立上下文，但也不能恢复 failed delegation，恢复只认后续成功的 write/run tool result；非 `done` run 中的失败 delegated result 也会以 warning 进入 completion report、Live Run Trace 和 replay audit。Live Run Trace 可查看 action id、round、sequence、字符计数、合同状态和失败类型等安全分发元数据；Live Run Trace 和 replay audit 还会保留 completion report 中 delegated completion-gate checks 的 id/status/summary/refs，覆盖 pass/warning/fail，但不会把 delegated output 当完成证明；replay audit 会对每轮 `delegate_agent` action 数和 delegated result 事件做覆盖检查，但不会读取或展示 raw delegated task/context/findings/output。
- delegated result 的 `summary/findings_text` 也不能自称已经调用工具、写入/突变状态、证明完成、调度专家/多 agent、执行 model fan-out、使用 hidden memory、读取 raw delegated artifact、依赖未声明 repo 状态、扩展 context 或自造 evidence ref；这类输出会作为 delegated output contract failure，并抑制 raw preview 后再回灌。
...
`result_failure_kind`、两层失败类型语义配对和每轮 active delegate 上限；缺少这些失败分类字段、合法枚举但配对错误，都会被标记为 warning，显式 `none` 才表示该层没有失败。该检查只读取有界 trace metadata，不读取 raw delegated task/context/findings/output。Replay JSON 会保留完整 delegated dispatch metadata 用于覆盖率审计；Markdown 或 context 展示可以只显示前几条并给出 omitted 计数。
...
`source_continuation` 会把 source layer/owner、已完成 source slice、source implementation contract、候选 next moves 数量、一个候选方向和主 `next_use` 保留成只读 handoff；它只保留来源方向，不执行下一步，也不证明完成；
...
`implementation_contract` 会在执行前说明本轮只允许一个可复用 GA design contract/read-model 改进、哪些外部工具/local-learning/专家调度范围要递延、以及交付标准是什么；当 slice 是 `general_agent_delegation`，执行合同的 source of truth 是 `packages/core/src/schemas.ts`、`packages/runtime/src/runner.ts`、Live Run Trace 和 replay audit，project-design/scorecard 只做计划和审计对齐；它是边界提示，不执行、不调度、不提升学习资产、不证明完成；
