...
`result_failure_kind`、两层失败类型语义配对和每轮 active delegate 上限；缺少这些失败分类字段、合法枚举但配对错误，都会被标记为 warning，显式 `none` 才表示该层没有失败；delegated completion-gate 的失败会在 replay check 中保持为 `fail`，顶层 replay report 仍以 `attention` 表示存在非 pass 检查。该检查只读取有界 trace metadata，不读取 raw delegated task/context/findings/output。Replay JSON 会保留完整 delegated dispatch metadata 用于覆盖率审计；Markdown 或 context 展示可以只显示前几条并给出 omitted 计数。
