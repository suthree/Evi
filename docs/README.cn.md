...
历史 Goal 仍可 read、pause、abandon，但不会被静默绑定后继续执行。
对 `codex.run`，runtime 还会在 pending effect 持久化前解析 new target 或 resume handle，
要求其真实 worktree、common dir、branch 和 delegated base 与 Goal 权威完全一致，并在
spawn 前再次校验；同一 common dir 下的 sibling worktree 也不能借用当前 Goal 的验证。
