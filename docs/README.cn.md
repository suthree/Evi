...
- `delegate_agent`：每个 model round 最多分发一个有界分析子任务，payload 只能包含 `task/context`，输出只能是 `summary/findings_text`；上限、failure kind 和 completion-gate check id 的共享字面合同在 `packages/core/src/action_contracts.ts`，schema、capability catalog、project-design 和 replay audit 从这里派生，runner 负责执行校验。trace/replay 只用安全元数据校验 `envelope_ref`、声明的 delegated action id、对应 sequence、persisted result ref 和 completion-gate status counts，不读取 delegated artifact body。它不是任务执行调度器，不能授予工具、状态写入、专家调度、模型 fan-out 或完成判断权；delegated result 只是 advisory self-report，不能作为 `completion_claim.verification_refs` 的完成证明。完整行为边界见 `docs/RUNTIME_CONTRACT.md` 的 `delegate_agent` 段和 `packages/runtime/src/runner.ts`。
...
`implementation_contract` 会在执行前说明本轮只允许一个可复用 GA design contract/read-model 改进、哪些外部工具/local-learning/专家调度范围要递延、以及交付标准是什么；当 slice 是 `general_agent_delegation`，字面合同的 source of truth 是 `packages/core/src/action_contracts.ts`，schema、runner、Live Run Trace、replay audit、project-design/scorecard 消费它做校验、执行和审计对齐；它是边界提示，不执行、不调度、不提升学习资产、不证明完成；
