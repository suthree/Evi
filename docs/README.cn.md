...
- `codex.run`：位于通用 `command.run` 之上的独立 typed coding execution tool。它只允许 `gpt-5.6-sol`、`fast` profile、`xhigh` reasoning、`fast` service tier、`read-only` 或 `workspace-write` sandbox，以及 `never` approval；会实时校验同一 Git common dir 下已注册的 sibling isolated worktree、base、branch 和 cwd，以不可变 digest 绑定 new/resume authority，持续校验 JSONL 并解析严格结构化状态，对整个 POSIX process group 做 TERM/KILL 清理，并从运行前后 live Git status 记录 tracked 与 untracked changed paths。`max_output_chars` 只限制证据保留：超限后不会因此终止进程或阻塞有效结果，metadata 会记录 observed、retained、truncated、实际 limit 与来源；调用者显式值优先，省略时从 active model `max_output_tokens` 或现有 context-budget fallback 派生。既有 v1 authority snapshot/resume 继续使用已持久化的实际预算。
...
本项目 runtime 将 `codex.run` 的 review、独立 diff/test 校验、权限、commit、PR、merge、deploy 与 completion authority 全部交由 `main_harness`；Codex 结果只作为执行证据。实现保持 capability-oriented、project-neutral，不在 `packages/`、配置或脚本中写入项目名。`codex.run` 与 advisory-only `delegate_agent` 严格分离，不改变后者的 payload、result、tool、write 或 completion authority。
