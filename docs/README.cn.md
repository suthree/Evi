...
前三个 vNext 源码切片已经证明 Kernel、reservation-first 的只读 Gateway，以及 terminal
reconciliation 后的显式 same-Run continuation，但尚未连接生产 ingress，也没有切换当前
v0.2 部署。
...
Adaptation 生命周期。当前源码只实现前三个基建切片，其中 Gateway 只允许只读 action，
支持 terminal reconciliation 后显式续跑同一 Run，但尚未切 ingress 或部署。稳定方向由
