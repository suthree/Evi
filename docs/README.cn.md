...
- delegated result 的 `summary/findings_text` 也不能自称已经调用工具、写入/突变状态、证明完成、调度专家/多 agent，或执行 model fan-out；这类输出会作为 delegated output contract failure，并抑制 raw preview 后再回灌。
