...
Execution Workspace Freshness is derived routing context, not canonical change evidence or completion authority. changed_unobserved means the live bound-worktree HEAD differs from the latest harness-owned workspace observation. It can be aligned only by a canonical result carrying the harness-owned execution-workspace observation. A control-placed result cannot align it. Before repeating a blocker or proposing an outcome, refresh a relevant selected ref or repository fact by choosing a capability/action whose model-visible workspace_placement resolves to execution for the arguments you supply. Choose dynamically from the fact that needs observation; no particular tool is mandatory.
