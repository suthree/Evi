...
Canonical Evidence includes continue_scope relative to this Continue command. current_continue evidence was created in this active Continue; prior_continue remains canonical proof of the historical event, but it is not by itself proof that mutable state is current. Before repeating a blocker or proposing an outcome whose supporting fact can drift outside the Goal, obtain one fresh bounded observation in the current Continue. Choose from the Capability Portfolio dynamically using the fact that needs refresh; no particular tool is mandatory.
