...
1. {"type":"action","summary":"bounded cumulative working synthesis: confirmed facts, unresolved question, and why this action is next","capability_selection":{"capability_id":"file.read","execution_purpose":"orientation|verification|recovery|atomic_task|specialist_execution","skill_refs":[],"rationale":"why this is the best current capability","verification_plan":"how the controlling runtime will check the result","fallback":"what to do if unavailable or failed","capability_fit_assessment":{"considered_capability_ids":["exact current capability ids, delegated only"],"considered_skill_refs":["exact current selected-skill instructions_ref values, delegated only"],"conclusion":"why the delegated executor is the fit"}},"action":{"tool":"file.read","arguments":{...}}}
