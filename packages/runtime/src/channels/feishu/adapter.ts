  ...
  private async renderContextManifestsCommand(contextRef?: string): Promise<string> {
    if (contextRef) {
      ...
      return [
        ...
        `budget_enforcement: ${result.manifest.budget_enforcement?.status ?? "not_recorded"}`,
        `budget_original_chars: ${result.manifest.budget_enforcement?.original_total_chars ?? result.manifest.total_chars}`,
        `budget_truncated_sections: ${result.manifest.budget_enforcement?.truncated_sections.length ?? 0}`,
        `budget_omitted_sections: ${result.manifest.budget_enforcement?.omitted_section_titles.length ?? 0}`,
  ...
  private async renderCompletionVerificationsCommand(completionRef?: string): Promise<string> {
    if (completionRef) {
      ...
      return [
        ...
        `verification_evidence_refs: ${report.verification_evidence_refs.length}`,
...
function renderContextManifestSummary(manifest: ContextManifestSummary, index: number): string[] {
  return [
    ...
    `   budget_enforcement: ${manifest.budget_enforcement_status}`,
    `   budget_original_chars: ${manifest.original_total_chars}`,
    `   budget_truncated_sections: ${manifest.truncated_section_count}`,
    `   budget_omitted_sections: ${manifest.omitted_section_count}`,
...
function renderCompletionVerificationSummary(
  ...
  return [
    ...
    `   verification_status: ${report.verification_status}`,
    `   verified: ${report.verified}`,
    `   verification_evidence_refs: ${report.verification_evidence_ref_count}`,
...
function renderLiveRunDelegatedDispatch(dispatch: LiveRunTraceSummary["delegated_dispatches"][number]): string[] {
  return [
    ...
    `  ok: ${dispatch.ok}`,
    `  model_invoked: ${dispatch.model_invoked ?? "unknown"}`,
...
function renderHarnessReplayDetail(replay: HarnessReplayAuditReport): string[] {
  return [
    ...
    `metrics: rounds=${replay.metrics.rounds}, events=${replay.metrics.events}, tool_results=${replay.metrics.tool_results}, delegated_failed=${replay.metrics.delegated_results_failed}, delegated_dispatches=${replay.metrics.delegated_dispatches}, delegated_dispatches_failed=${replay.metrics.delegated_dispatches_failed}, verification_evidence_refs=${replay.metrics.verification_evidence_refs}, repo_write_guards=${replay.metrics.repo_write_guards}`,
