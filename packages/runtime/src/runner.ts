...
export class LiveAgentRunner {
  ...
  private async executeDelegation(action: ModelActionEnvelope["actions"][number], round: number, sequence: number): Promise<DelegatedResult> {
    ...
    try {
      ...
      const summary = sanitizeModelDiagnosticText(parsed.summary, DELEGATED_AGENT_SUMMARY_MAX_CHARS);
      const findingsText = sanitizeModelDiagnosticText(parsed.findings_text, DELEGATED_AGENT_FINDINGS_MAX_CHARS);
      return delegatedResultSchema.parse({
        id: newId("delegated_result"),
        ok: true,
        summary,
...
function verifyCompletionClaim(args: {
  ...
  const claimedSuccessfulWriteOrRunRefs = args.verificationEvidenceRefDrafts
    .filter((draft) => draft.is_write_run && boundClaimedRefs.includes(draft.ref))
    .map((draft) => draft.ref);
