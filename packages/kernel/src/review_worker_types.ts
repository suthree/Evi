...
const MAX_REVIEW_PACKET_BYTES = 96 * 1024;
...
export async function captureReviewEvidencePacket(
  subjectInput: ExecutionWorkerInspection
  ...
  const observed = await captureDeliveryLineageSnapshot(subject.lineage);
  assertSameSnapshotState(expected, observed, subject.id);
  ...
  const observedAfterCapture = await captureDeliveryLineageSnapshot(subject.lineage);
  assertSameSnapshotState(expected, observedAfterCapture, subject.id);
  const body = {
  ...
  if (Buffer.byteLength(stableJson(body), "utf8") > MAX_REVIEW_PACKET_BYTES) {
    throw new Error(`Review evidence packet exceeds ${MAX_REVIEW_PACKET_BYTES} bytes: ${subject.id}`);
  }
...
export function materializeReviewResultEnvelope(
  ...
  if (input.status === "completed") {
    if (input.verdict === null || ((input.verdict === "approved") !== (findings.length === 0))) {
...
function assertSameSnapshotState(
...
async function readGitEntry(
  ...
  const result = await execFileAsync("git", ["show", `${commit}:${path}`], {
    cwd: worktree,
    encoding: null,
    maxBuffer: MAX_REVIEW_PACKET_BYTES + 1,
    timeout: 15_000
...
async function readWorktreeEntry(
  ...
  try {
    ...
    if (info.size > MAX_REVIEW_PACKET_BYTES) {
      throw new Error(`Review evidence file exceeds ${MAX_REVIEW_PACKET_BYTES} bytes: ${path}`);
    }
    return {
